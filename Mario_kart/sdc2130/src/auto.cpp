...
{
printf("A!@#\n");
device.SetCommand(_G,2,-800);
device.SetCommand(_P,1,-1000);
sleep(1);
device.SetCommand(_P,1,0);
}

if(temp<0)
{
device.SetCommand(_G,2,-800);
printf("bitch i am so horny\n");
device.SetCommand(_P,1,1000);
sleep(1);
device.SetCommand(_P,1,0);
}
...
maxes[5]=joy->axes[5];
 ...
 void pathcallback(const turtlesim::Pose::ConstPtr& path)
  {
  x=path->x;
  y=path->y;
  }
  void   encocallback(const turtlesim::Pose::ConstPtr& track)
  {
   ref=track->x;
...
ros::Subscriber sub1=nh.subscribe("/path",1000,pathcallback);
ros::Subscriber sub2=nh.subscribe("/encoY/coory",1000,encocallback);
...
//module for aligning the robot based on the encoder value
...
if(sets==0)//B
{
//if we have a free path,relocate the wheel to center to position and resume procedure
if(temp>0)
{
device.SetCommand(_G,2,-600);//-700 for actual  movement
device.SetCommand(_P,1,-100);
usleep(600000);//600 milliseconds delay.
device.SetCommand(_P,1,0);
}
if(temp<0)
{
...
{
device.SetCommand(_G,2,-600);//for actual movement set to -700
device.SetCommand(_P,1,sets);
usleep(600000);//600 milliseconds delay.
device.SetCommand(_P,1,0);
temp=sets;
}
...
{
//wall avoidance manuever
printf("\nreverse");
device.SetCommand(_G,2,800);//go reverse in case of wall or too many obstacles in path
sleep(4);
device.SetCommand(_P,1,-100);
usleep(600000);//600 milliseconds delay.
device.SetCommand(_P,1,0);
temp=-100;
//align straight and proceed
y=0;
}
printf("\n%d",sets);
...
if(i>10)
{
 cout<<" Rear wheel:"<<rw<<" front pos:"<<fs<<" Encoder Value:"<<result<<endl;
 i=0;
}
*/
//sleep(1);
